...
*   **Fix Critical Issues**: Address the core functionality failures in Invoice Builder and Template Builder pages (blocking issues)
...
## 6. Known Issues
*   [x] **Critical: Invoice Builder Page Not Loading** - Invoice Builder page fails to load with missing form elements (`input[placeholder*="Invoice Number"]` element not found). This blocks invoice creation, editing, and PDF generation.
...
*   [x] **Critical: PDF Generation Timeout** - PDF generation tests timeout due to dependency on Invoice Builder functionality (30-second timeout waiting for invoice number input).
...
| Invoice Builder            | [`tests/invoice-builder.spec.ts`](tests/invoice-builder.spec.ts) | ❌ **CRITICAL ISSUES** | 0% | Core functionality failing. Invoice Builder page does not load (invoice number input not found). All tests dependent on initial load failed or skipped. |
| PDF Generation             | [`tests/pdf-generation.spec.ts`](tests/pdf-generation.spec.ts) | ❌ **CRITICAL ISSUES** | 0% | Dependent on Invoice Builder. Tests timeout waiting for invoice number input. Cannot test PDF generation without functional invoice builder. |
...
| Customer/Product Management| [`tests/customer-product-management.spec.ts`](tests/customer-product-management.spec.ts) | ⚠️ **PARTIAL** | 75% | Basic functionality works (addition via invoice builder), but mobile viewport line item UI missing. Product management UI not fully implemented. |

**Overall Test Coverage Status:** ❌ **INSUFFICIENT** - Critical functionality issues prevent comprehensive testing of core features (Invoice Builder, Template Builder, PDF Generation).
